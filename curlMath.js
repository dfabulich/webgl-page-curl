...
export function calculateCurledVertexPosition(originalX, originalY, geomWidth, geomHeight, amount) {
  ...
  if (u <= u_peel_front && u >= -0.0001) {
    // Allow for small floating point inaccuracies for u starting at 0
    ...
    if (averageRadius > 0.0001) {
      // Avoid division by zero if H (and thus R) is tiny
